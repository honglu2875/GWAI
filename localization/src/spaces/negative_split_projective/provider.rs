...
pub(crate) fn twisted_genus_zero_three_primary_value_coeff<C: Coeff>(
    ...
    let left_cyclic_coordinates =
        quantum_cyclic_coordinates_coeff(&quantum_h, &insertions[0], degree)?;
    let product = quantum_product_from_left_cyclic_coordinates_coeff(
        &quantum_h,
        &left_cyclic_coordinates,
        &insertions[1],
        degree,
    );
...
/// Coefficients in the q-dependent basis `(1, H, H*H, ...)` generated by
/// quantum multiplication, rather than in the provider's fixed flat basis.
struct QuantumCyclicCoordinates<C: Coeff>(Vec<QSeries<C>>);

fn quantum_cyclic_basis_matrix_coeff<C: Coeff>(
    quantum_h: &SeriesMatrix<C>,
    q_degree: usize,
) -> SeriesMatrix<C> {
    let colors = quantum_h.rows();
    debug_assert_eq!(quantum_h.cols(), colors);
    let mut columns = Vec::with_capacity(colors);
    let mut current = vec![QSeries::<C>::zero(q_degree); colors];
    current[0] = QSeries::<C>::one(q_degree);
    for _ in 0..colors {
        columns.push(current.clone());
        current = apply_series_matrix_to_vector_coeff(quantum_h, &current, q_degree);
    }
...
fn quantum_cyclic_coordinates_coeff<C: Coeff>(
    quantum_h: &SeriesMatrix<C>,
    vector: &[QSeries<C>],
    q_degree: usize,
) -> Result<QuantumCyclicCoordinates<C>, GwError> {
    let basis = quantum_cyclic_basis_matrix_coeff(quantum_h, q_degree);
    let inverse = invert_series_matrix_coeff(&basis).map_err(|error| match error {
        GwError::NonSemisimplePoint => GwError::ConventionMismatch(
            "twisted quantum H multiplication does not generate an invertible cyclic basis"
                .to_string(),
        ),
        other => other,
    })?;
    Ok(QuantumCyclicCoordinates(
        apply_series_matrix_to_vector_coeff(&inverse, vector, q_degree),
    ))
}

fn quantum_product_from_left_cyclic_coordinates_coeff<C: Coeff>(
    quantum_h: &SeriesMatrix<C>,
    left_cyclic_coordinates: &QuantumCyclicCoordinates<C>,
    right: &[QSeries<C>],
    q_degree: usize,
) -> Vec<QSeries<C>> {
    let colors = quantum_h.rows();
    debug_assert_eq!(left_cyclic_coordinates.0.len(), colors);
    debug_assert_eq!(right.len(), colors);
    let mut result = vec![QSeries::<C>::zero(q_degree); colors];
    let mut h_power_right = right.to_vec();
    for (power, left_coeff) in left_cyclic_coordinates.0.iter().enumerate() {
        if !left_coeff.is_structurally_zero() {
            for row in 0..colors {
                result[row] = result[row].add(&h_power_right[row].mul(left_coeff));
            }
        }
        if power + 1 < left_cyclic_coordinates.0.len() {
            h_power_right =
...
mod provider_tests {
    ...
    fn noncyclic_quantum_h_reports_a_cyclic_basis_mismatch() {
        ...
        let vector = vec![QSeries::<Rational>::one(0), zero];

        let error = match quantum_cyclic_coordinates_coeff(&quantum_h, &vector, 0) {
